...
//}
#define log_i 	usb_printf	//打印信息
#define log_e  	usb_printf	//打印信息
...
static int accel_self_test(long *bias_regular, long *bias_st)
{
    ...
    for(jj = 0; jj < 3; jj++) {
        ...
        if (st_shift[jj]) 
        {
          ...
          {
            usb_printf("st_shift_var:%f > %f\n",st_shift_var,test.max_accel_var);
            result |= 1 << jj;
          }
         } 
        else if ((st_shift_cust < test.min_g) || (st_shift_cust > test.max_g))
        {
          usb_printf("st_shift_cust:%f (%f,%f)",st_shift_cust,test.min_g,test.max_g);
            result |= 1 << jj;
        }
    }
...
int mpu_run_self_test(long *gyro, long *accel)
{
    ...
    if (ii == tries) {
        /* Again, probably an I2C error. */
			usb_printf("ii == tries 22222\t\n");
        result = 0;
        goto restore;
    }
    accel_result =0 ;//accel_self_test(accel, accel_st);
    usb_printf("accel_self_test_result= %d\n",accel_result );
    gyro_result =0;// gyro_self_test(gyro, gyro_st);
    usb_printf("gyro_self_test_result= %d\n",gyro_result );
...
uint8_t run_self_test(void)
{
	...
	result = mpu_run_self_test(gyro, accel);
        usb_printf("mpu_run_self_test_result=%d\n", result);
	if (result == 0x3) 
	{
...
uint8_t mpu_dmp_init(void)
{
  ...
  {	 
    ...
    {
			usb_printf("1F\t\n");
      usb_printf("set sensors failed \n");
      return 1; 
    }
    else usb_printf("set sensors successful \n");
    res=mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);//设置FIFO
    if(res)
    {
      usb_printf("set FIFO failed \n");
      return 2; 
    }
    else usb_printf("set FIFO successful \n");
    res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//设置采样率
    if(res)
    {
      usb_printf("set sample rate failed \n");
      return 3;
    }
    else usb_printf("set sample rate successful \n");
    res=dmp_load_motion_driver_firmware();		//加载dmp固件
    if(res)
    {
      usb_printf("load DMP failed \n");
      return 4;
    }
    else usb_printf("load DMP successful \n");
    res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//设置陀螺仪方向
    if(res)
    {
      usb_printf("set orientation failed \n");
      return 5; 
    }
    else usb_printf("set orientation successful \n");
    res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	//设置dmp功能
        DMP_FEATURE_ANDROID_ORIENT|DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
        DMP_FEATURE_GYRO_CAL);
    if(res)
    {
      usb_printf("set DMP failed \n");
      return 6; 
    }
    else usb_printf("set DMP successful \n");
    ...
    if(res)
    {
     usb_printf("self test failed \n");
     return 8; 
   }   
    else usb_printf("self test successful \n");
    res=mpu_set_dmp_state(1);	//使能DMP
    if(res)
    {
      usb_printf("enable DMP failed \n");
      return 9; 
    }
    else usb_printf("enable DMP successful \n");
  }
	usb_printf("res = %d\t\n",res);
  usb_printf("initialize MPU6050 successful \n");
  return 0;
}
...
uint8_t mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	{
		usb_printf("dmp_read_fifo error\t\n");
		return 1;	 
	}
	...
	**/
	...
	{
		usb_printf("sensors&INV_WXYZ_QUAT error\t\n");
		return 2;
	}
		return 0;
}
